...
var prefetch = (src, options) => {
  ...
  fetch(src, {
    ...
    Log.trace(logLevel, `[prefetch] Finished prefetch ${src} with method ${method}`);
    objectUrl = url;
    setPreloads((p) => ({
      ...p,
      [src]: objectUrl
...
var LogLevelContext = createContext14("info");
var useLogLevel = () => {
...
var seek = ({
  ...
  Log.trace(logLevel, `[seek] ${mediaRef.src} from ${mediaRef.currentTime} to ${timeToSet}. Reason: ${why}`);
...
var useMediaBuffering = ({
  ...
  useEffect9(() => {
    ...
    if (isPremounting) {
      if (current.readyState < current.HAVE_FUTURE_DATA) {
        if (!navigator.userAgent.includes("Firefox/")) {
          Log.trace(logLevel, `[load] Calling .load() on ${current.src} because readyState is ${current.readyState} and it is not Firefox. Element is premounted`);
          current.load();
        }
      }
      return;
    ...
    const blockMedia = (reason) => {
      setIsBuffering(true);
      Log.trace(logLevel, `[buffer] buffering ${current.src}. reason = ${reason}`);
      ...
      cleanupFns.push((cleanupReason) => {
        Log.trace(logLevel, `unblocking ${current.src} from buffer. reason = ${cleanupReason}`);
        unblock();
      });
    };
    const init = () => {
      if (current.readyState < current.HAVE_FUTURE_DATA) {
        ...
        if (!navigator.userAgent.includes("Firefox/")) {
          Log.trace(logLevel, `[load] Calling .load() on ${current.src} because readyState is ${current.readyState} and it is not Firefox.`);
          current.load();
        }
      } else {
        const onWaiting = () => {
...
var VideoForDevelopmentRefForwardingFunction = (props2, ref) => {
  ...
  useState18(() => Log.trace(logLevel, `[video] Mounting video with source = ${actualSrc}`));
  useEffect21(() => {
